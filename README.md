...
**专为跨性别群体（Transgender）打造的~~离线优先~~ (待开发)、极简开源实用工具箱**
...
在信息获取日益困难、医疗资源分配不均的当下，TP 旨在成为一个"装在口袋里的庇护所"。~~它完全支持离线运行，确保你的核心知识和生理数据不依赖于任何云端服务器，永远掌握在自己手中。~~ (纯离线架构持续迭代中，当前 Beta 版部分功能需联网)
...
### 📚 知识库 (Wiki)
* ~~内嵌精编版 MtF/FtM 生存指南与 HRT 基础知识。~~ (本地物理打包待开发)
* ~~首次使用时需联网缓存 Wiki 页面内容。~~ (离线数据引擎待开发)
* ~~之后每次开启时：若不联网则自动显示本地缓存版本。~~ (静默热更新机制待开发)
* **当前 Beta 版状态**：采用 WebView 纯联网模式，需保持网络畅通以实时加载 mtf.wiki 等开源指南。

### 💊 药物存量仪表盘 & 本地用药提醒
* 追踪 HRT 药物库存存量与安全续航天数，含量化面板与药物列表。
* **Chronos 智能调度引擎**：支持小时、天、周、月四种给药周期——从口服（12h）到针剂（7天）到 GnRHa（28天/84天）。
* **锚点日期系统**：基于绝对时间精准注册本地通知，长效药物永不错过。
* 自动推算逻辑：点击"已服药"自动扣减库存、推算下一次给药时间并重设系统闹钟。
* 隐私安全：该模块采用纯本地 JSON 持久化与系统原生推送，断网可用。
...
* 支持质量浓度与摩尔浓度的智能单位切换，自动过滤等价单位。
* **跨旗色彩符号系统**：参考范围卡片采用跨性别旗帜色彩（MtF 粉 / FtM 蓝 / NB 中性色），匹配时高亮弹起。

### 🎙️ 声音训练辅助
* 基于开源项目 [VFS Tracker](https://github.com/Ethanlita/vfs-tracker) 集成优化。
* 包含嗓音测试、音阶练习、88键钢琴、F0检测、主观量表与鼓励机制。
* 生成本地音频分析报告与训练记录时间线。
* ~~可扩展 AWS 云端后端同步数据。~~ (云端同步模块待开发)
...
1. **非医疗建议：** 本应用提供的所有功能、文本、图表及**尤其是"血药浓度模拟器"得出任何计算结果，均仅供学术交流与数据可视化参考。** 它**绝对不能**替代专业内分泌科医生的诊断、处方或临床医学检验。
2. **个体差异：** 药代动力学（PK）算法中的常量取自开源社区经验值与公开医学文献的平均水平。**每个人的肝肾代谢酶活性、体重、吸收率存在极其巨大的个体差异。** 模拟器画出的完美曲线，绝不代表您体内的真实浓度。
3. **责任豁免：** 本项目的开发者、贡献者不对用户依据本应用提供的数据进行的任何"自我药疗（DIY HRT）"行为及其产生的任何生理、心理后果承担任何形式的法律和医疗责任。
4. **遵医嘱：** 调整激素剂量是一项严肃的医疗行为，请务必在正规医生的指导下并结合实际的血液化验结果进行。
...
* 感谢 **[Project Trans](https://project-trans.org/)** 及 **[MtF.wiki](https://mtf.wiki/)** 团队，为中文跨性别社群提供了极其宝贵的开源知识库。本项目的 Wiki 理念及激素换算参考范围均离不开前人的卓越工作。
